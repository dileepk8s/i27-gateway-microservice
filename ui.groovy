def deployToEnv (String namespace, String envLabel) {
  ...
  sh '''
      ...
      echo "Applying Kubernetes manifests in ${envLabel} namespace"
      kubectl apply -f k8s/
      echo "Deployment to ${envLabel} Environment completed successfully!"
    '''
}
def gkeAuth (String clusterName, String clusterZone, String clusterProject) {
  sh """
      echo "*****************Authenticating to GKE Cluster*****************************"
      gcloud container clusters get-credentials ${clusterName} --zone ${clusterZone} --project ${clusterProject}
...
pipeline {
  ...
  stages {
    ...
    stage ('Deploy To Dev Environment') {
      ...
      steps {
        script {
          //calling auth function to authenticate to the cluster
          //String clusterName, String clusterZone, String clusterProject
          gkeAuth(env.DEV_CLUSTER_NAME, env.DEV_CLUSTER_ZONE, env.DEV_PROJECT_ID)
          //calling deployToEnv function to deploy the application into dev environment
